...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # السماح لجميع الأصول مؤقتًا
...
try:
    ...
    client.server_info()  # سيُلقي استثناء إذا لم يتمكن من الاتصال
...
try:
  ...
  users_collection.create_index([("phoneNumber", 1)])
  users_collection.create_index([("full_name", 1)])  # إضافة فهرسة على full_name
...
class Order(BaseModel):
    user_id: str
    products: List[Dict[str, Union[str, int]]]  # تغيير لقبول str أو int
    total_price: float
    payment_method: str
    delivery_method: str
    status: str = "Pending"
...
async def manual_login(form_data: OAuth2PasswordRequestForm = Depends()):
  try:
    print(f"Starting manual_login for phoneNumber: {form_data.username}")
    print(f"Fetching user from database...")
    db_user = users_collection.find_one({"phoneNumber": form_data.username})
    if not db_user:
      print(f"Phone number {form_data.username} not found in database")
      raise HTTPException(status_code=400, detail="رقم الهاتف أو كلمة المرور غير صحيحة")
    print(f"Found user: {db_user}")
    print("Verifying password...")
    if not pwd_context.verify(form_data.password, db_user["hashed_password"]):
      print(f"Password verification failed for phoneNumber {form_data.username}")
      raise HTTPException(status_code=400, detail="رقم الهاتف أو كلمة المرور غير صحيحة")
    print("Checking if user is active...")
    if not db_user.get("is_active", True):
      print(f"User with phoneNumber {form_data.username} is inactive")
    ...
    user_data = {
      ...
      "is_superuser": db_user.get("is_superuser", False),
      "subscription": db_user.get("subscription", False),
...
async def register_user(user_data: dict):
  ...
  required_fields = ["full_name", "password", "phoneNumber"]
  ...
  if not phone_number.startswith("966") or len(phone_number) != 12:
    print(f"Invalid phone number format: {phone_number}")
    raise HTTPException(status_code=400, detail="رقم الهاتف يجب أن يبدأ بـ 966 ويكون طوله 12 رقمًا")
  existing_phone = users_collection.find_one({"phoneNumber": phone_number})
  if existing_phone:
    print(f"Phone number already in use: {phone_number}")
    raise HTTPException(status_code=400, detail="رقم الهاتف مستخدم بالفعل")

  print(f"محاكاة: تم إرسال رمز OTP إلى {phone_number}")
  print("محاكاة: الرمز الافتراضي هو 123456")
  ...
  return {"message": "تم إنشاء الحساب بنجاح، يرجى تسجيل الدخول", "user_id": str(result.inserted_id)}
...
async def add_product(
    ...
    product_dict = {
        ...
        "price": price,
        "stock": int(stock),  # التأكد من أن stock يُخزن كـ int
        "category": category,
        "brand": brand,
    ...
    response_dict = product_dict.copy()
    response_dict["id"] = str(result.inserted_id)
    response_dict.pop("_id", None)
    return {"message": "Product added successfully", "product": response_dict}
# مسار لتعديل منتج
...
async def create_order(order: Order, current_user: User = Depends(current_active_user)):
    ...
    for product in order_dict["products"]:
        ...
        # تحويل stock إلى عدد صحيح
        stock = int(db_product["stock"]) if isinstance(db_product["stock"], str) else db_product["stock"]
        if stock < quantity:
            raise HTTPException(status_code=400, detail=f"Product {product_id} is out of stock")
        # تحويل trending_score إلى عدد صحيح إذا كان موجودًا
        trending_score = int(db_product["trending_score"]) if isinstance(db_product["trending_score"], str) else db_product["trending_score"]
        # تحديث المخزون والـ trending_score
        products_collection.update_one(
            {"_id": ObjectId(product_id)},
            {"$set": {"stock": stock - quantity, "trending_score": trending_score + quantity}}
        )

    # معالجة الدفع
    if order.payment_method == "Tamara":
        order_dict["payment_id"] = "tamara_123"  # محاكاة الدفع
    elif order.payment_method == "Tabby":
        order_dict["payment_id"] = "tabby_123"  # محاكاة الدفع
    ...
    if order.delivery_method == "SMSA":
        order_dict["tracking_number"] = "smsa_123"  # محاكاة التوصيل
    elif order.delivery_method == "Aramex":
        order_dict["tracking_number"] = "aramex_123"  # محاكاة التوصيل
    else:
        order_dict["tracking_number"] = "site_pickup"

    try:
        result = orders_collection.insert_one(order_dict)
        # إنشاء قاموس جديد للاستجابة بدون حقل _id
        response_dict = order_dict.copy()
        response_dict["id"] = str(result.inserted_id)
        response_dict.pop("_id", None)  # إزالة حقل _id

        # تحديث purchase_history
        users_collection.update_one(
            {"id": order.user_id},
            {"$push": {"purchase_history": response_dict}}
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to create order: {str(e)}")

    return {"message": "Order created successfully", "order": response_dict}
...
async def add_to_favorites(product_id: str, current_user: User = Depends(current_active_user)):
    product = products_collection.find_one({"_id": ObjectId(product_id)})  # إزالة القوس الإضافي
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
