...
async def manual_login(form_data: OAuth2PasswordRequestForm = Depends()):
    try:
        print("Using modified manual_login version - main2")
        print(f"Starting manual_login for username: {form_data.username}")
        ...
        print("Converting to User object")
        user_data = {
            "id": str(db_user["_id"]),
        ...
        print(f"JWT Strategy created: {jwt_strategy}")
        ...
        # تحديد نوع المستخدم (عميل أو مسؤول)
        user_type = "admin" if user.is_superuser else "customer"

        # إضافة شرط خاص لـ naeem
        if form_data.username == "naeem" and user.is_superuser:
            print("User naeem detected - redirecting to admin pages")
            user_type = "admin"
        ...
        return JSONResponse(content={
            ...
            "token_type": "bearer",
            "user_type": user_type,
            "full_name": user.username,  # إضافة full_name
            "email": user.email,        # إضافة email
